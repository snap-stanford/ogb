...
After training models using the script, there will be prediction files dumped under the `$TEST_DIR`. The prediction files are in the following format: `[valid/test]_$PROCID_$STEP`, e.g., `test_0_99999.pkl`, `test_1_99999.pkl`, which means the test prediction files generated by training on two GPUs at step 99999. Then please use the following code to save the test submission file based on the best valid performance.
...
```
This will save the test submission file at `${TEST_DIR}/t_pred_wikikg90m.npz`.
