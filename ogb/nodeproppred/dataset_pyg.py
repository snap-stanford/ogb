...
from ogb.io.read_graph_pyg import read_bin_graph_pyg
...
class PygNodePropPredDataset(InMemoryDataset):
    def __init__(self, name, root="dataset", transform=None,
        ...
        self.dir_name = "_".join(
            name.split("-")
        ) + "_pyg"  ## replace hyphen with underline, e.g., ogbn_proteins_pyg
        ...
        if osp.isdir(self.root) and (not osp.exists(
                osp.join(
                    self.root, 'RELEASE_v' +
                    str(self.meta_info[self.name]['version']) + '.txt'))):
    ...
    def process(self):
        ...
        if self.meta_info[self.name]["binary"] == "True":
            data = read_bin_graph_pyg(self.raw_dir, add_inverse_edge)
