...
def split_test(split_dict):
    if isinstance(split_dict['test'], torch.Tensor):
        ...
        competition_idx = np.nonzero(~(idx % 5 < 3))[0]

    split_dict['test-whole'] = split_dict['test']
    split_dict['test-dev'] = split_dict['test-whole'][dev_idx]
    split_dict['test-challenge'] = split_dict['test-whole'][competition_idx]

    assert len(split_dict['test-dev']) + len(split_dict['test-whole'][competition_idx]) == len(split_dict['test'])
